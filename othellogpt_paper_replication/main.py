...
def apply_scale(resid: Float[Tensor, "batch=1 seq d_model"], 
                flip_dir: Float[Tensor, "d_model"], scale: int, pos: int):
    '''
...
def calculate_attn_and_mlp_probe_score_contributions(
    ...
    d_model, rows, cols = my_probe.shape
    attn_contributions = t.zeros((layer+1, rows, cols))
    mlp_contributions = t.zeros((layer+1, rows, cols))
...
def calculate_neuron_input_weights(
    ...
    '''
    return einops.einsum(
        probe, get_w_in(model, layer, neuron, True),
...
def calculate_neuron_output_weights(
    ...
    '''
    return einops.einsum(
        probe, get_w_out(model, layer, neuron, True),
        "d_model row col, d_model -> row col"
    )
...
# %%
c0_unembed = model.W_U[:,to_int("C0")].detach()
d1_unembed = model.W_U[:,to_int("D1")].detach()

cos_sim = t.nn.functional.cosine_similarity(x1=c0_unembed, x2=d1_unembed, dim = 0)
print(cos_sim)
...
# 10xrowxcol
output_weights_in_logit_basis = t.zeros(10,8,8)

W_out = model.W_out[layer, top_neurons, :].detach().clone()
W_out /= W_out.norm(dim = -1, keepdim=True)
output_weights_in_logit_basis.flatten(start_dim=1)[:,stoi_indices] = einops.einsum(
    W_out, model.W_U[:,1:],
    "neuron d_model, d_model vocab -> neuron vocab"
)
...
neuron_acts = focus_cache["post", layer, "mlp"][:, :, top_neurons]
top_neuron_values = neuron_acts.flatten(end_dim=2).quantile(0.99, dim=0)

top_moves = neuron_acts > top_neuron_values
...
for i in range(10):
    board_states[i] = focus_states_flipped_pm1[:, :-1][top_moves[:,:,i]].float().mean(0)
...
# %% Exercise: Investigate this plot
...
class LitLinearProbe(pl.LightningModule):
    ...
    def training_step(self, batch: Int[Tensor, "game_idx"], batch_idx: int) -> t.Tensor:
        ...
        print(state_stack.shape) # 256, 60, 8, 8

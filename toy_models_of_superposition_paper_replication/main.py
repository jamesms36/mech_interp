...
class Model(nn.Module):
    ...
    def forward(
        ...
        WX = einops.einsum(
            self.W, features,
            "n_instances n_hidden n_features, ... n_instances n_features -> ... n_instances n_hidden"
        )
        WTWX = einops.einsum(
            self.W, WX,
            "n_instances n_hidden n_features, ... n_instances n_hidden -> ... n_instances n_features"
        )
        out = WTWX + self.b_final
        return F.relu(out)
...
def calculate_loss(
    ...
    '''
    return einops.reduce(model.importance*((out- batch).pow(2)), "b i f -> i", "mean").sum()
...
class Priviledged_Model(Model):
    ...
    def generate_batch(self, n_batch) -> Tensor:
        return super().generate_batch(n_batch)
...
optimize(pmodel)
plot_Ws_from_model(pmodel, config)
